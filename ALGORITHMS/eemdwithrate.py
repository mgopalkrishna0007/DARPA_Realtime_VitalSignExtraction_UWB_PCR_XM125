...
from PyEMD import EMD  # Import Empirical Mode Decomposition
...
ax[0].set_title("Real-Time Phase & EMD Components")
ax[0].grid(True)
...
def update(frame_idx):
    ...
    # **Apply EMD every 100 frames and update the same window**
    if frame_idx % 5 == 0 and frame_idx > 0:  
        emd = EMD()
        imfs = emd(phi[:frame_idx])
